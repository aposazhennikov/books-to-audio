...
SYNTHESIS_TRANSLATIONS: dict[str, dict[str, str]] = {'synth.no_manifest': {'en': 'No manifest loaded', 'ru': 'Манифест не загружен'},
 ...
 'synth.audio_output': {'en': 'Audio files:',
                        'ru': 'Аудиофайлы:',
                        'zh': '音频文件：',
                        'kk': 'Аудио файлдар:',
                        'uz': 'Audio fayllar:'},
 'synth.audio_output_status': {'en': 'Synthesis writes WAV chunks and creates compatible MP3 sidecars when ffmpeg is available.',
                               'ru': 'Синтез записывает WAV-чанки и создаёт совместимые MP3-копии, если доступен ffmpeg.',
                               'zh': '合成会写入 WAV 分块；如果 ffmpeg 可用，还会创建兼容的 MP3 副本。',
                               'kk': 'Синтез WAV чанктарын жазады және ffmpeg қолжетімді болса, үйлесімді MP3 көшірмелерін жасайды.',
                               'uz': 'Sintez WAV bo‘laklarini yozadi va ffmpeg mavjud bo‘lsa, mos MP3 yon nusxalarini yaratadi.'},
 'synth.audio_output_help': {'en': 'This pipeline currently renders chunk audio as WAV. A receiver-compatible MP3 sidecar is exported best-effort with ffmpeg and recorded in the manifest.',
                             'ru': 'Текущий pipeline сохраняет аудио чанков в WAV. Совместимая MP3-копия создаётся через ffmpeg по возможности и записывается в манифест.',
                             'zh': '当前流水线将分块音频渲染为 WAV。兼容接收设备的 MP3 副本会尽量通过 ffmpeg 导出，并记录到清单中。',
                             'kk': 'Қазіргі pipeline чанк аудиосын WAV ретінде шығарады. Қабылдағышқа үйлесімді MP3 көшірме ffmpeg арқылы мүмкіндігінше экспортталып, manifest ішіне жазылады.',
                             'uz': 'Hozirgi pipeline bo‘lak audiosini WAV sifatida chiqaradi. Qabul qiluvchiga mos MP3 yon nusxasi ffmpeg orqali imkon qadar eksport qilinadi va manifestga yoziladi.'},
 'synth.audio_output_internal_both': {'en': 'WAV + MP3 sidecar',
                                      'ru': 'WAV + MP3-копия',
                                      'zh': 'WAV + MP3 副本',
                                      'kk': 'WAV + MP3 көшірме',
                                      'uz': 'WAV + MP3 yon nusxa'},
 'synth.audio_output_internal_wav': {'en': 'WAV only',
                                     'ru': 'Только WAV',
                                     'zh': '仅 WAV',
                                     'kk': 'Тек WAV',
                                     'uz': 'Faqat WAV'},
 ...
 'synth.batch_unavailable_help': {'en': 'Current synthesis pipelines render chunks sequentially, so batch size is fixed at 1.',
                                  'ru': 'Текущие pipeline синтеза рендерят чанки последовательно, поэтому размер батча зафиксирован на 1.',
                                  'zh': '当前合成流水线按顺序渲染分块，因此批量大小固定为 1。',
                                  'kk': 'Қазіргі синтез pipeline чанктарды ретімен рендерлейді, сондықтан batch өлшемі 1 болып бекітілген.',
                                  'uz': 'Hozirgi sintez pipeline bo‘laklarni ketma-ket render qiladi, shuning uchun partiya hajmi 1 ga mahkamlangan.'},
